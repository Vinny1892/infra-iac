...
generate "main" {
  ...
  contents  = <<-EOF
    ...
    # IP publico reservado criado SEM vinculo com VNIC.
    # O endereco fica conhecido em tempo de apply, antes da instancia existir,
    # o que permite injeta-lo no user_data (--tls-san) sem descoberta em runtime.
    # A associacao com a VNIC e feita pela unit applications/compute/vm.
    ...
    output "public_ip_address" {
      description = "Endereco IPv4 reservado, estavel entre ciclos de destroy/deploy da VM"
...
inputs = {
  ...
  display_name   = "k3s-regulus-reserved-ip"
}
