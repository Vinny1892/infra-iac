...
- **Testing**: Terratest (Go v0.46.16) for unit testing with mock providers.
...
Managed via Terragrunt `dependency` blocks:
...
make lint-tflint     # tflint only (uses .tflint.hcl with AWS ruleset v0.35.0)
...
- **`terragrunt.hcl`**: Every leaf directory (unit) contains exactly one `terragrunt.hcl` which defines the `source` module, `inputs`, and often generates the `main.tf` inline. **Do not edit .tf files manually; they are generated by Terragrunt.**
...
Before running commands, export the required environment variables:
```bash
export AWS_PROFILE=personal
export CLOUDFLARE_API_TOKEN=<token>

# For K3s helms (ArgoCD bootstrap)
...
- Tests live in `tests/` and use **Terratest v0.46.16** (Go).
- All tests use the `//go:build unit` tag and mock providers to avoid real API calls.
- **Test types**: **Validate** (HCL syntax/types) and **Plan** (structural checks).
- **K3s organism test**: Requires running `scripts/extract-k3s-fixture.sh` once before execution to sync the fixture at `tests/fixtures/k3s_extracted/`.
...
### K3s Organism Implementation Notes
- **User-data**: Scripts in `organisms/aws/k3s/cluster/scripts/`.
- **Init Master**: Installs `k3s server`, connects to RDS via Secrets Manager, uploads JWKS to S3, saves kubeconfig to SSM.
- **Init Worker**: Installs `k3s agent` only, points to NLB. Uses `k3s-worker-role` with least privilege.
- **GitHub App**: Terraform generates private key via `github_app_private_key` for ArgoCD bootstrap.
